	...
	factory: (require) => {
		...
		*/
		function fuzzyMatchSkill(item, rawQuery) {
			...
			if (nameLower.startsWith(query)) return {
				item,
				score: 1e3 + Math.max(0, 100 - nameLower.length),
				nameHighlights: [{
					start: 0,
					end: query.length
				}],
		...
		*/
		function matchAcronym(target, query) {
			...
			const initials = [];
			for (let i = 0; i < target.length; i++) if ((i === 0 || target[i - 1] === "-" || target[i - 1] === "_" || target[i - 1] === " " || target[i - 1] === "/") && target[i] >= "a" && target[i] <= "z") initials.push({
				char: target[i],
				index: i
			});
		...
		function matchSubsequence(target, query) {
			...
			while (tIdx < target.length && qIdx < query.length) {
				if (target[tIdx] === query[qIdx]) {
					if (tIdx === 0 || target[tIdx - 1] === "-" || target[tIdx - 1] === "_" || target[tIdx - 1] === " ") score += 35;
		...
		*/
		...
		const zh = { "menu.userOnly": "仅用户" };
		const en = { "menu.userOnly": "user-only" };
		//#endregion
		...
		function apply(ctx) {
			...
			const source = {
				...
				async candidates(session, { query, signal }) {
					...
					const pickedSkillNames = new Set(Array.from(currentText.matchAll(/\/([a-zA-Z0-9_-]+)/g)).map((m) => m[1]));
					...
					}).filter((item) => item !== null).sort((a, b) => b.score - a.score).map(({ item }) => ({
						name: item.name,
